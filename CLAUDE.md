...
- **Zodiac frames:** `HouseCusp` carries `siderealDegree` **and** `tropicalDegree`; `PlanetPosition.absoluteDegree` means "longitude in the frame named by `ChartResult.system`". Never pair a longitude from one frame with cusps from another — that was a live bug (SP-0) that put 8 of 12 planets in the wrong house. Cosmic Passport, `/sidereal-dates`, `/why-sidereal` and all SEO surfaces stay sidereal regardless of the viewer's toggle.
