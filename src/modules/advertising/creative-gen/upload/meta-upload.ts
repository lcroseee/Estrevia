...
// ---------------------------------------------------------------------------
// AI Content Label helper
// ---------------------------------------------------------------------------

/**
 * Returns true if the creative was generated by an AI image/video model and
 * therefore requires Meta's AI Content Label on upload (per Meta's March 2026
 * AI labeling policy). Satori is deterministic templating (@vercel/og), not
 * AI — exclude it. All other current and future generators in the union are
 * AI by default; if a new non-AI generator is added, extend this helper.
 */
export function isAiGenerated(generator: GeneratedAsset['generator']): boolean {
  return generator !== 'satori';
}
...
export async function uploadApprovedCreative(
  ...
  const metaResult = await deps.metaApi.uploadCreative({
    ...
    tracking,
    is_ai_generated: isAiGenerated(bundle.asset.generator),
  });
